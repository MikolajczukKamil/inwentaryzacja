...
*/
DROP 
  TABLE IF EXISTS asset_types;
DROP 
  TABLE IF EXISTS rooms;
DROP 
  TABLE IF EXISTS assets;
DROP 
  TABLE IF EXISTS reports;
DROP 
  TABLE IF EXISTS reports_assets;
DROP 
  TABLE IF EXISTS users;
DROP 
  TABLE IF EXISTS buildings;
DROP 
  TABLE IF EXISTS login_sessions;
...
/*
DROP 
  FUNCTION IF EXISTS getRoomIdWithAsset;
/*
    *   If the asset is not allocated function returns -1
    
DELIMITER $$ CREATE FUNCTION getRoomIdWithAsset(asset_id INT) RETURNS INT BEGIN DECLARE room_id INT DEFAULT -1;
SELECT 
...
INSERT INTO reports_assets (
  report_id, asset_id, previous_room
) 
VALUES 
  (1, 127, 30), 
  (1, 128, 30), 
