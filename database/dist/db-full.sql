DROP TABLE IF EXISTS scans_positions; DROP TABLE IF EXISTS scans; DROP TABLE IF EXISTS reports_positions; DROP TABLE IF EXISTS reports; DROP TABLE IF EXISTS assets; DROP TABLE IF EXISTS rooms; DROP TABLE IF EXISTS buildings; DROP TABLE IF EXISTS asset_types; DROP TABLE IF EXISTS login_sessions; DROP TABLE IF EXISTS users;
CREATE TABLE users (
...
REFERENCES assets (id)
ON DELETE CASCADE ) ENGINE = InnoDB; INSERT INTO asset_types (letter, name) VALUES ('c', 'komputer'),
('k', 'krzesło'),
('m', 'monitor'),
('p', 'projektor'),
('s', 'stół'),
('t', 'tablica') ;
INSERT INTO buildings (name) VALUES ('b 34'),
('b 4'),
('b 5'),
('b 6'),
('b 7'),
...
('b 32'),
('b 33') ;
INSERT INTO rooms (name, building) VALUES ('3/6', 1),
('3/40', 1),
('3/19', 1),
('1/2', 2),
('1/3', 3),
...
('1/8', 8),
('1/9', 9) ;
INSERT INTO assets (type) VALUES
(1), (2), (3), (4), (5), (6), (1), (2), (3), (4), (5), (6), (1), (2), (3), (4), (5), (6), (1), (2), (3), (4), (5), (6), (1), (2), (3), (4), (5), (6), (1), (2), (3), (4), (5), (6), (1), (2), (3), (4), (5), (6), (1), (2), (3), (4), (5), (6), (1), (2), (3), (4), (5), (6), (1), (2), (3), (4), (5), (6) ;
INSERT INTO users (login, hash) VALUES ('user1', '$2y$10$GefRLqURuebo9eyOrYm83O0zFeGlvVt3UujXgDd02gyjZwH6NO6N6'),
...
INSERT INTO reports (name, room, owner, create_date) VALUES ('Raport 1', 1, 1, NOW() - INTERVAL 10 DAY), ('Raport 2', 2, 1, NOW() - INTERVAL 9 DAY), ('Raport 3 po 1', 1, 1, NOW() - INTERVAL 8 DAY), ('Raport 4', 3, 1, NOW() - INTERVAL 7 DAY), ('Raport 5 po 3', 1, 1, NOW() - INTERVAL 6 DAY), ('Raport 6', 4, 1, NOW() - INTERVAL 5 DAY) ;
INSERT INTO reports_positions (report_id, asset_id, previous_room, present) VALUES
(1, 0 + 1, NULL, TRUE),
(1, 0 + 2, NULL, TRUE),
(1, 0 + 3, NULL, TRUE),
(1, 0 + 4, NULL, TRUE),
...
(2, 6 + 5, NULL, TRUE),
(2, 6 + 6, NULL, TRUE),
(3, 0 + 1, 1, TRUE), (3, 0 + 2, 1, TRUE), (3, 0 + 3, 1, TRUE), (3, 0 + 4, 1, TRUE), (3, 0 + 5, 1, FALSE), (3, 0 + 6, 1, FALSE), (3, 6 + 1, 2, TRUE), (3, 6 + 2, 2, TRUE), (4, 12 + 1, NULL, TRUE),
(4, 12 + 2, NULL, TRUE),
(4, 12 + 3, NULL, TRUE),
(4, 12 + 4, NULL, TRUE),
(4, 12 + 5, NULL, TRUE),
(4, 12 + 6, NULL, TRUE),
(5, 0 + 1, 1, TRUE), (5, 0 + 2, 1, TRUE), (5, 0 + 3, 1, FALSE), (5, 0 + 4, 1, FALSE), (5, 6 + 1, 1, TRUE), (5, 6 + 2, 1, TRUE), (5, 12 + 1, 3, TRUE), (5, 12 + 2, 3, TRUE), (5, 6 + 3, 2, FALSE), (5, 6 + 4, 2, FALSE), (6, 18 + 1, NULL, TRUE),
(6, 18 + 2, NULL, TRUE),
(6, 18 + 3, NULL, TRUE),
(6, 18 + 4, NULL, TRUE),
(6, 18 + 5, NULL, TRUE),
(6, 18 + 6, NULL, TRUE) ;
INSERT INTO login_sessions (user, token, expiration_date, create_date) VALUES (1, 'fake-token', NOW() + INTERVAL 1 YEAR, NOW() - INTERVAL 1 DAY) ;
INSERT INTO scans (room, owner, create_date) VALUES (1, 1, NOW());
INSERT INTO scans_positions (scan, asset, state) VALUES (1, 1, 0),
(1, 2, 0),
(1, 3, 0),
(1, 4, 0),
(1, 5, 0),
(1, 6, 0),
(1, 7, 0),
(1, 15, 0) ; DROP FUNCTION IF EXISTS assetExists;
DELIMITER $ CREATE FUNCTION assetExists(_id INT) RETURNS BOOLEAN BEGIN
RETURN (SELECT COUNT(*) FROM assets WHERE assets.id = _id) = 1; END $ DELIMITER ; DROP FUNCTION IF EXISTS asset_typeExists;
DELIMITER $ CREATE FUNCTION asset_typeExists(_id INT) RETURNS BOOLEAN BEGIN
RETURN (SELECT COUNT(*) FROM asset_types WHERE asset_types.id = _id) = 1; END $ DELIMITER ; DROP FUNCTION IF EXISTS buildingExists;
DELIMITER $ CREATE FUNCTION buildingExists(_id INT) RETURNS BOOLEAN BEGIN
RETURN (SELECT COUNT(*) FROM buildings WHERE buildings.id = _id) = 1; END $ DELIMITER ; DROP FUNCTION IF EXISTS getRoomIdWithAsset;
...
RETURN IF(Have_duplicates, NULL, CONCAT(Table_name, ' zawiera dublikaty (id = ', Ids, ')')); END $ DELIMITER ; DROP FUNCTION IF EXISTS idsNotFound;
DELIMITER $ CREATE FUNCTION idsNotFound(Table_name VARCHAR(32), Ids VARCHAR(1024), Is_found BOOLEAN) RETURNS VARCHAR(64) BEGIN
RETURN IF(Is_found, NULL, CONCAT(Table_name, ' nie istnieje (id = ', Ids, ')')); END $ DELIMITER ; DROP FUNCTION IF EXISTS reportExists;
DELIMITER $ CREATE FUNCTION reportExists(_id INT) RETURNS BOOLEAN BEGIN
RETURN (SELECT COUNT(*) FROM reports WHERE reports.id = _id) = 1; END $ DELIMITER ; DROP FUNCTION IF EXISTS roomExists;
DELIMITER $ CREATE FUNCTION roomExists(_id INT) RETURNS BOOLEAN BEGIN
RETURN (SELECT COUNT(*) FROM rooms WHERE rooms.id = _id) = 1; END $ DELIMITER ; DROP FUNCTION IF EXISTS scanExists;
DELIMITER $ CREATE FUNCTION scanExists(_id INT) RETURNS BOOLEAN BEGIN
...
RETURN (SELECT COUNT(*) FROM users WHERE users.id = _id) = 1; END $ DELIMITER ; DROP PROCEDURE IF EXISTS addNewAsset;
